module.exports = async ({ core, exec, context, fetch }, token) => {
  ...
  try {
    ...
    await exec.exec('git', ['config', '--local', 'user.name', 'gr-oss-devops charts'], { cwd: checkoutPageDir })
    ...
    return core.setFailed(`Unable to push ${checkoutPageDir}/${helm.charts.destination} to gr-oss-devops/charts@${branchName}\nError: ${error}`)
  } finally {
